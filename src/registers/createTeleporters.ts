...
export async function genTeleporterRegistry(inputPath: string, outputPath: string) {
  ...
  const teleporters = parseCsv<SpreadsheetColumns>(await readFile(path.join(inputPath, "teleporter.csv")))
    .slice(1)
    .filter(data => data[1] !== "")
  ...
  teleporters.forEach(({ id, dimension, pos, activationState, color, groups }) => {
  ...
  writeFile(path.join(outputPath, "early_register.json"), JSON.stringify({ values: teleporters.map(({ id }) => `asset:teleporter/${id}/early_register`) }));
}
